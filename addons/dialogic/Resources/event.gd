...
func get_property_translated(property_name:String) -> String:
	if !_translation_id.is_empty() and ProjectSettings.get_setting('dialogic/position/enabled', false):
		var position = tr(get_property_translation_key(property_name))
		# if no translation is found tr() returns the id, but we want to fallback to the original
		return position if position != get_property_translation_key(property_name) else _get_property_original_translation(property_name)
	else:
