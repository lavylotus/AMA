...
func set_indent(indent: int) -> void:
	add_theme_constant_override("offset_left", indent_size*indent)
...
func _set_event_icon(icon: Texture2D) -> void:
...
func build_editor(build_header:bool = true, build_body:bool = false) ->  void:
	...
	for p in resource.get_event_editor_info():
		...
		## INTEGERS
			...
			editor_node.set('theme_override_colors/font_color', Color("#7b7b7b"))
...
func _ready():
	...
	%Header.add_theme_constant_override("theme_override_constants/separation", 5 * _scale)
...
func _on_ExpandButton_toggled(button_pressed:bool) -> void:
	...
	body_container.add_theme_constant_override("offset_left", icon_size*DialogicUtil.get_editor_scale())
