...
func _draw() -> void:
	...
	for idx in range($Timeline.get_child_count()):
		...
		var position :Vector2= event.get_node('%IconPanel').global_position+Vector2(0,1)*event.get_node('%IconPanel').size+Vector2(0,-4)
		...
		if idx < $Timeline.get_child_count()-1 and event.current_indent_level < $Timeline.get_child(idx+1).current_indent_level:
			...
			var size := Vector2()
			if end_node != null:
				size = Vector2(line_width, end_node.global_position.y+end_node.size.y-position.y)
				if end_node.resource is DialogicEndBranchEvent and event.resource.can_contain_events:
					size = Vector2(line_width, end_node.global_position.y+end_node.size.y/2-position.y)
			else:
				size = Vector2(line_width, $Timeline.get_child(-1).global_position.y+$Timeline.get_child(-1).size.y-position.y)

			draw_rect(Rect2(position-global_position, size), color)
			draw_rect(Rect2(Vector2(event.get_node('%IconPanel').global_position.x+line_width, position.y+size.y-line_width)-global_position, Vector2(horizontal_line_length, line_width)), color)

		elif event.expanded:
			draw_rect(Rect2(position-global_position, Vector2(line_width, event.size.y-event.get_node('%IconPanel').size.y+10*_scale)), color.darkened(0.5))
