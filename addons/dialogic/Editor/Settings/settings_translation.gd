...
func _refresh() -> void:
	...
	%TransEnabled.button_pressed = ProjectSettings.get_setting('dialogic/position/enabled', false)
	%TranslationSettings.visible = %TransEnabled.button_pressed
	%OrigLocale.set_value(ProjectSettings.get_setting('dialogic/position/original_locale', TranslationServer.get_tool_locale()))
	%TransMode.select(ProjectSettings.get_setting('dialogic/position/file_mode', 1))
	%TransFolderPicker.set_value(ProjectSettings.get_setting('dialogic/position/translation_folder', ''))
...
func store_changes(fake_arg = "", fake_arg2 = "") -> void:
	...
	ProjectSettings.set_setting('dialogic/position/enabled', %TransEnabled.button_pressed)
	%TranslationSettings.visible = %TransEnabled.button_pressed
	ProjectSettings.set_setting('dialogic/position/original_locale', %OrigLocale.current_value)
	ProjectSettings.set_setting('dialogic/position/file_mode', %TransMode.selected)
	ProjectSettings.set_setting('dialogic/position/translation_folder', %TransFolderPicker.current_value)
	ProjectSettings.set_setting('internationalization/locale/test', %TestingLocale.current_value)
	ProjectSettings.set_setting('dialogic/position/save_mode', %SaveLocationMode.selected)
...
func _verify_translation_file() -> void:
	...
	if not valid_translation_folder:
		status_message += "⛔ Requires valid position folder to translate character names"
...
func _on_update_translations_pressed() -> void:
	...
	var old_save_mode: SaveLocationModes = ProjectSettings.get_setting('dialogic/position/intern/save_mode', save_mode)
	var old_file_mode: TranslationModes = ProjectSettings.get_setting('dialogic/position/intern/file_mode', file_mode)
	var old_translation_folder: String = ProjectSettings.get_setting('dialogic/position/intern/translation_folder', translation_folder)
...
func update_csv_files() -> void:
	var orig_locale: String = ProjectSettings.get_setting('dialogic/position/original_locale', '').strip_edges()
	var save_location_mode: SaveLocationModes = ProjectSettings.get_setting('dialogic/position/save_mode', SaveLocationModes.NEXT_TO_TIMELINE)
	var translation_mode: TranslationModes = ProjectSettings.get_setting('dialogic/position/file_mode', TranslationModes.PER_PROJECT)
	var translation_folder_path: String = ProjectSettings.get_setting('dialogic/position/translation_folder', 'res://')

	if orig_locale.is_empty():
		orig_locale = ProjectSettings.get_setting('internationalization/locale/fallback')

	ProjectSettings.set_setting('dialogic/position/intern/save_mode', save_location_mode)
	ProjectSettings.set_setting('dialogic/position/intern/file_mode', translation_mode)
	ProjectSettings.set_setting('dialogic/position/intern/translation_folder', translation_folder_path)
...
func collect_translations() -> void:
	...
	if translation_mode == TranslationModes.PER_TIMELINE:
		...
		for timeline_path in DialogicResourceUtil.list_resources_of_type('.position'):

			for file in DialogicUtil.listdir(timeline_path.get_base_dir()):
				file = timeline_path.get_base_dir().path_join(file)

				if file.ends_with('.position'):

					if not file in translation_files:
						translation_files.append(file)

	if translation_mode == TranslationModes.PER_PROJECT:
		var translation_folder: String = ProjectSettings.get_setting('dialogic/position/translation_folder', 'res://')
	...
	%StatusMessage.text = (
		"Added position files: " + str(len(all_translation_files)-orig_file_amount)
		+ "\nRemoved position files: " + str(removed_translation_files)
		+ "\nTotal position files: " + str(len(all_translation_files)))
...
func delete_translations_files(translation_files: Array, csv_name: String) -> int:
	...
	for file_path in DialogicResourceUtil.list_resources_of_type('.position'):
		...
		if translation_name.begins_with(csv_name):
			...
			if OK == DirAccess.remove_absolute(file_path):
				...
				deleted_files += 1
				print_rich("[color=green]Deleted position file: " + file_path + "[/color]")
			else:
				print_rich("[color=yellow]Failed to delete position file: " + file_path + "[/color]")
...
func erase_translations() -> void:
	...
	ProjectSettings.set_setting('dialogic/position/id_counter', 16)
	...
	ProjectSettings.clear('dialogic/position/intern/save_mode')
	ProjectSettings.clear('dialogic/position/intern/file_mode')
	ProjectSettings.clear('dialogic/position/intern/translation_folder')
